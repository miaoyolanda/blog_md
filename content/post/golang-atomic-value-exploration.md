...
那为什么在上面的场景中，`atomic`会比`mutex`性能好很多呢？作者 [Dmitry Vyukov](http://www.1024cores.net/home/about-me) 总结了这两者的一个区别：
...
{{< figure src="/image/golang-atomic-value/64-bit-write.png" caption="64位变量的赋值操作" >}}
...
面对这种多线程下变量的读写问题，我们的主角——`atomic.Value`登场了，它使得我们可以不依赖于不安全的`unsafe.Pointer`类型，同时又能将任意数据类型的读写操作封装成原子操作（让中间状态对外不可见）。
...
出于安全考虑，Go 语言并不支持直接操作内存，但它**内部** 同时又提供一种*不安全* 的指针类型`unsafe.Pointer`，让程序可以灵活的读取/操作内存。
...
1. 第一次写入（第9~24行） - 一个`Value`实例被初始化后，它的`typ`字段会被设置为指针的零值 nil，所以第9行先判断如果`typ`是 nil 那就证明这个`Value`还未被写入过数据。那之后就是一段初始写入的操作：
   * `runtime_procPin()`这是runtime中的一段函数，具体的功能我不是特别清楚，也没有找到相关的文档。这里猜测一下，一方面它禁止了调度器对当前 goroutine 的抢占（preemption），使得当前 goroutine 可以尽快完成工作，因为别人一直在等待它。另一方面，在禁止抢占期间，GC 线程也无法被启用，这样可以防止 GC 线程看到一个莫名其妙的指向`^uintptr(0)`的类型（这是赋值过程中的中间状态）。
...
本文从邮件列表中的一段讨论开始，介绍了`atomic.Value`的被提出来的历史缘由。然后由浅入深的介绍了它的使用姿势，以及内部实现。让读者不仅知其然，还能知其所以然。
